...
/* =====================================================================
   ...
   Asserts the §6.6 "verified behaviour" plus structural invariants that must
   hold for EVERY breed. The assertions below are UNCHANGED from the version
   that tested the standalone tool — that is the whole point: the 2026-08-03
   port to the Astro page /breed-matcher/ moved the engine into modules and
   changed nothing about how it scores, and this file is the proof.

   Run:  npm run test:breed-matcher
         (or)  node tools/breed-matcher/test/engine.test.mjs

   ---------------------------------------------------------------------
   WHAT CHANGED IN THE PORT (2026-08-03)

   It used to read public/breed-matcher/index.html, extract the inline
   <script> with lastIndexOf, stub the DOM, and (0,eval) the lot with the
   assertions appended to the same string so they could see the eval'd
   consts. All of that machinery existed only because the tool was one HTML
   file. It now imports the real modules:

     src/scripts/breed-matcher/data.js    BREEDS (96 rows) + QUESTIONS
     src/scripts/breed-matcher/engine.js  scoreBreed / whyLine / buildTips
     src/scripts/breed-matcher/services.js the service links the copy names

   ⚠️ Those three modules are deliberately PLAIN JS with no imports from
   src/data/*.ts, so this harness can load them with a bare `node`. If you
   ever add a TypeScript import to any of them, this test stops running —
   which means the scoring engine stops being covered. Don't.
   ===================================================================== */

import { BREEDS } from '../../../src/scripts/breed-matcher/data.js';
import { scoreBreed } from '../../../src/scripts/breed-matcher/engine.js';
