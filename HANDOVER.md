...
- **STATUS: on disk (OneDrive-synced), NOT committed, NOT pushed.** Owner reviews on the live preview → needs a push to `main`. **Do not push until the owner asks** (push = deploy). Next: owner review → swap soul content → polish pass (Lighthouse ≥90 / a11y / reduced-motion audit) → live enquiry-form n8n test (service defaults; CORS from a real browser) → sign-off → tick tracker row 12 → **site feature-complete → Stage 5 cutover checklist**.
