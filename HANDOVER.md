...
- **Adversarial review** (6 dimensions x 3 refuters, 30 agents) → fixed: chip end-state dim `0.4`→`0.6` (chip text was 3.16:1, group heads 2.80:1 — now 5.38:1 / 4.70:1); caption `Goldie` alpha `.55`→`.75` (4.15:1 → 6.63:1); outcome eyebrow `.62`→`.75`; outcome headline **honey-300 → cream-50** (halo + headline + CTA were three stacked honey focal points, blunting the button); group names `<p>` → `<h3>` + `<ul aria-labelledby>`; `"Doggy day school"` → `"Dog day school"` (matches the product name used elsewhere on the page); sub-line rewritten so it no longer promises only "skills" when the 4th group is observation; `<Image sizes>` corrected to the real 240px box; `saveData` gate + early-out added to the rAF loop; unused `--color-clay-400` dropped; SVG ids namespaced per instance.
- **KNOWN TRADE-OFF (documented, not fixed):** on the pinned desktop path the outcome block + its CTA are `visibility:hidden` (GSAP `autoAlpha`) until the scrub reaches ~65%, so a keyboard user who tabs straight through never focuses that CTA. This is correct WCAG behaviour (no invisible focus targets) and the same `#enquire` CTA appears in the hero, the reassurance section and the enquiry form; reduced-motion users see everything at once. Revisit at polish if the owner wants.
