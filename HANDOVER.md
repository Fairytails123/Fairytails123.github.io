...
- `/puppycourse` built → the six `astro.config.mjs` stubs (`resources-collection`, `puppy-week-1/3/4`, `week-2-puppy`, `puppy-toilet-schdule`) → `/puppycourse` (now `/blog`) **+** the puppy-training-classes page's "Do-it-yourself puppy course" pathway card → `/puppycourse` (now `/blog`, `TEMP:` comment in `puppy-training-classes.astro`).
...
## ▶ 2026-07-05 — 🐶 PAGE 4 /puppy-training-classes BUILT (interview → spec → 4 media assets → page + site-wide wiring)
...
- **OWNER REVIEW LIST for this page:** (1) class-night rundown times/blocks (draft), (2) kit list (draft), (3) FAQ answers esp. missed-session + family-attendance wording (drafts), (4) the "Intensive Training" nav label, (5) supply the puppy-parent testimonial when ready, (6) week 3 recall wording ("earns them their freedom") ok?
