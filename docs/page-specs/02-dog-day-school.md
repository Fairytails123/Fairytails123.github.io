...
3b. **The curriculum rain (added 2026-07-11, owner-directed — `<DaySchoolRain />`)** — interactive word-rain diorama between the enrichment pillars and the day-in-the-life story, on the same cream-100 canvas: all ~100 real day-school services (the owner's 106-service handoff list, tightened) fall as Fraunces words over a meadow scene; hover slows/warms a word, landings ripple. **No mascot** — the design-handoff dog/fairy/catch-game were removed at owner instruction ("just a rain"). SSR static scatter = the no-JS/reduced-motion scene. Word list + section copy are DRAFTS in `src/components/DaySchoolRain.astro`.
...
## BUILD STATE ADDENDUM (2026-07-11 — section 4 "curriculum rain" added)

The owner's `design_handoff_service_rain_hero` animation (landed 2026-07-11) was ported **natively** into this page as new **section 4** (`<DaySchoolRain />`, between "Dogs were built for a busier world" and "A day in the life") — owner resolved the HANDOVER's open questions: this page (not the membership hero), fully re-skinned to the countryside system. **Mid-build the owner cut the dog mascot ("dog animation is very basic and doesn't suit the brand's premium look — just a rain")**, so the fairy + catch-game went with it: the shipped section is a pure typographic rain — 100 real services falling as Fraunces words (moss-800/bark-700 inks, honey-700 hover, depth = size/opacity/blur/speed, cursor-repel, moss landing ripples) over a cream diorama with meadow floor + hills. SSR static scatter serves no-JS/reduced-motion/save-data identically. Engine is vanilla rAF (no GSAP), IO + visibility gated, torn down on `astro:before-swap`; DEV-only `window.__rain.step()` drives it for screenshots (absent from dist). QA: build 33 pages clean · verify-urls 48/0 · desktop 1920 + true-390 iframe + sandboxed no-JS iframe all screenshot-verified · zero console errors. **Owner still owes:** section copy review (eyebrow/H2/sub + the 100-word list — all DRAFT), and a real-phone look at the rain density/feel. **Owner corrected 3 tiles later the same day:** "No bullying, ever" → "No dog bullies others" · "New surfaces underfoot" → "Puppies explore new areas" · "Beach days" → "Dog walks" (grammar normalised, orthography-only convention).

**Master-pool placement (2026-07-11, `82d49c3`):** the welcome-section polaroid ("A proper day out — for dogs") is now master image `4` — a playroom birthday party (portrait, staff holding the cake, class gathered under HAPPY BIRTHDAY bunting) — as `src/assets/pages/dog-day-school/welcome-party.webp`, replacing the quadruple-used `doggy-friends` (which stays on this page's rail card). Caption "Fellow furry friends" kept. Live-verified.

**Estimator membership nudge (2026-07-11, `186f0b6`):** the cost estimator now shows parents when a membership plan beats their configuration (owner ask — "3+ days a week every week"). Computed live, never hard-coded: full days compare against Tier 1 (£375/mo), half days against Half Days (£275/mo), + £25/mo large-dog where set — on the fees membership replaces (day fees + pick-up/drop-off, free on every plan; Sniffari excluded from both sides since it cancels). When the plan wins by £1+, a honey note in the result panel names the plan, its all-in price and the floor-rounded monthly saving, linking to `/membership-plans`. Verified across 7 configurations (e.g. full 5d weekly = save £58; full 4d weekly hidden; 4d weekly + bus = save £6 — the owner's "more than 3 days" crossover; half 5d = save £49; large 5d = save £249; sniffari toggle leaves the saving unchanged). Live-verified.
