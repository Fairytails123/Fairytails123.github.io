...
> **🔴 SUPERSEDING CORRECTION 2026-07-24 — "FORCE-FREE" IS BANNED SITE-WIDE. Every mention of it below is HISTORY, not an instruction.** Owner directive: we are **pain-free dog trainers**, never "force-free", because a lead, a legally required muzzle or a nail trim all involve a degree of *force* — so "force-free" is a claim we cannot honestly keep — while none of them involve *pain*. The site must also name the three tools we never use: **choke chains, prong collars, electric e-collars**. Shipped as `src/components/PainFreePromise.astro` on 9 pages. **On this page specifically:** the meta description at line 100, the trust-strip bullet at line 44 and the PROOF credibility card all shipped with "force-free" and were **changed on 2026-07-24**. ✅ **The accreditation + qualification signals STAY — owner-confirmed 2026-07-24:** *"keep accredited and qualified trainers as it is important trust factor."* A review had flagged "Accredited," and "Recognised trainer & behaviourist qualifications" as unevidenced; it was briefly removed and **has been restored at the owner's instruction**. The distinction that matters: this is a **documentation gap in the repo, not a false claim** — the owner holds the facts. 🔶 **Still open (line 124): the awarding body's NAME**, which would let it join the council licence in the `LocalBusiness` `hasCredential` schema and turn a bare claim into a verifiable one. Note the accreditation is no longer described as *force-free* accreditation — the term is banned as a self-description (locked #14). Note the still-standing rule that this spec already records at line 44: never frame training as countryside-only.
...
1. **HERO — the 3D transformation moment (signature).** Full-viewport. Eyebrow *"Dog training · day school · board & train — in Hastings Country Park"*. Headline carries the transformation (DRAFT options, owner picks one):
   ...
   Sub-line (DRAFT): *"Kind, real-world training and day school that turns nervous, reactive or overwhelmed dogs into calmer, happier companions — set in the heart of the Sussex countryside."* Primary CTA **"Start your enquiry"** (smooth-scrolls to the enquiry form / `#enquire`); soft secondary **"Or talk to us"** → `tel:` / WhatsApp `business.phones.main`. Trust strip (3 `.paw-bullet`): *force-free, qualified trainers · real-world countryside setting · daily reports home*. **⚠️ 2026-07-15 copy correction (owner):** the live hero no longer frames training as countryside-*located* — the trust bullet shipped as **"Countryside base, real-world training"** and the flagship blurb as "…**trains out in the real world**". Dogs *settle/board* at the countryside base but *train in busy real-world places* (town, cafés, traffic); the draft's "real-world countryside setting" / "in the heart of the Sussex countryside" wording was the mismatch. See CLAUDE.md / SEO.md §8 absolutes. The WebGL "anxious → calm" scene sits behind/around the copy (§Hero technical plan). **Static fallback = poster frame + all copy fully visible** (no JS needed).
...
5. **TWO TRACKS — the services spine (signature structural device).** A visually distinct split; scroll-driven reveal.
   - **Track A — "Training that transforms"** (leads; for struggling dogs). Two **flagship feature blocks**: **Board & Train £2,500** (+£300 large) → `/dog-boarding-school`, **Intensive £2,000** (+£300 large) → `/intensive-dog-training`; then a tidy grid for the rest → detail pages: ~~**puppy versions £1,200**~~ → **the puppy course £1,200 (singular — see the note below)**, **1-to-1 £49.99**, **Puppy Classes £175**, **free phone consult**. Carries the exact-case **`#Trainingplans`** anchor summary (renders from `pricing.json`, `board-train` + `training` categories). Enquiry-first CTAs.
     - 🔴 **2026-07-21 — there is only ONE £1,200 puppy programme now.** The owner confirmed the two were one product (*"they're one product"*); **`puppy-intensive` was deleted** from `pricing.json` and absorbed into **`board-train-puppy`** (`publicName` "Puppy School"). Because Track A renders straight from the two categories, the grid **self-corrected to one puppy card** — but `index.astro`'s `detailHref` map needed the **`puppy-intensive` key dropped** and the puppy route now hangs off `board-train-puppy` → `/comprehensive-puppy-training`. **When the homepage is finally built, write "the puppy course" (singular) and render its title as `publicName ?? name`** so the card, its CTA and the page it lands on all say "Puppy School". Never re-create a second £1,200 puppy card.
...
**Third master-pool placement (2026-07-11, `3354bc6`):** image `8` — the playroom birthday party — replaced the club-life band's right-hand image (was the two-dog `happy-walk`, now the `/membership-plans` hero at owner instruction). Asset `src/assets/pages/home/club-party.webp` (1200×900, 16:10 `object-[50%_65%]`). The left barn image is unchanged.
