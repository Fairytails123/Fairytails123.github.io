...
7. **Pricing** (renders from `src/data/pricing.json`) — Puppy £1,200 / 4 wks · Adult behaviour £2,500 / 8–10 wks (+£300 large) · Holiday Board & Train £1,500 / 4 wks (+£300 large).
   - 🔴 **2026-07-21 — the puppy tier here is the SAME PRODUCT as `/comprehensive-puppy-training`.** The owner confirmed the two £1,200 puppy programmes are one (*"they're one product"*), so `puppy-intensive` was deleted and **`board-train-puppy` is the single residential puppy course**: formal `name` **"Boarding School for Puppies"** (what this page shows, as one tier of the **range**) and `publicName` **"Puppy School"** (what its dedicated page shows). **Owner's framing: Board & Train is the residential RANGE — older dogs AND puppies, varying lengths and entry criteria — and the puppy course is one of its tiers.** A range page presenting a tier is **not** a competing page (SEO.md #12 clause 3 does not fire: `serviceType`, `areaServed` and offer counts all differ). **Whether the two URLs should eventually consolidate (Option C) is an OPEN OWNER DECISION, deferred until the freeze lifts.**
   - ⚠️ **FREEZE RULE:** this page renders `board-train-puppy`'s **`name` / `price` / `unit` / `eligibility` / `features`** — **do not change those fields while the drop-recovery freeze holds.** Adding *non-rendered* keys (`publicName`, `arrangements`, `_mergeNote`) is freeze-safe; that is exactly how the merge shipped without disturbing this page. The boarding rule (`arrangements`) is **not** rendered here.
   - 📌 **Freeze disclosure (2026-07-21):** across three deploys this page's rendered output changed by **exactly two lines** — the enquiry dropdown option (3 words → 2, inside a form control) — plus a schema-only `Offer` `@id` addition (`<domain>/#offer-<id>`), the same non-confounding class as the 2026-07-16 Service+Offer work. Verified beforehand: visible-text diff vs live = **0 changed lines**.
   - 🎯 **When the freeze lifts, the first thing that ships here is a named, priced PUPPY TIER band** — drafted ready-to-paste in `SEO.md` §7a (now the top-scoring item on the SEO board: the homepage is currently taking the puppy-boarding queries and converting none of them — figures in the gitignored `SEO.md` §7, not repeated here per locked #7). Ship it in ONE batch with the 3 parked alt strings + this page's `updated` date, then re-pull GSC filtered on "puppy". **Do not ship it piecemeal** — the batch is the measurement.
