...
> **Primary term = "puppy school" (150/mo, KD 8), Track L (local).** *(2026-07-18: the product is no longer local-only — see the fact correction below. The term and the Track L assignment **stay as recorded** by explicit owner deferral; flagged for review, not for change.)* The `<title>`, H1 area and hero eyebrow lead with **"Puppy School"** — the name the site's nav already used while the title buried the page under "Comprehensive Puppy Training", a phrase with **~0 search demand**. `pricing.json` keeps the formal product name; **that duality is deliberate.** The **slug is unchanged** — term battles are fought in title/H1/copy, never the slug (SEO.md decision #1).
...
11. **FAQ** (cream-50) — 6 DRAFT FAQs incl. "how is this different from classes / Board & Train". ⚠️ **The "vs Board & Train" answer can no longer say "we're the one where your pup comes home" (owner correction 2026-07-18 — both board). That answer is blocked on the OPEN QUESTION above.**
12. **Other puppy pathways** (cream-100) — cross-sell (minus self): Puppy Training Classes (£175 parent-led), Boarding School for Puppies (£1,200 — ~~**distinguished** from this full-day £1,200 product~~ **❓ 2026-07-18: the old "residential vs full-day" contrast is FALSE now that both products board. The real distinction is an OPEN QUESTION awaiting the owner — do not invent one, and do not let this card's copy assert the dead contrast**), free DIY course (**TEMP → /blog** until `/puppycourse` ships).
