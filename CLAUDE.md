...
- **Service links** in the tool (`FT = {…}`) were repointed to the NEW site 2026-07-04 (the old-Duda paths 404'd after cutover). Only `FT.puppy` is still TEMP (`/#Trainingplans`) — flip to `/puppy-training-classes` when that page ships (see the HANDOVER flip-back checklist).
