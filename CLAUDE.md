...
- `src/data/pricing.json` — `offerings[]`, each `{id, category, name, price, unit, largeDogSurcharge, eligibility, features[], acuityUrl, order, weeklyDiscountPrice?}`. Pages **filter by `category`** (`board-train` | `training` | `day-school` | `membership`) and **sort by `order`**; format money with a local `gbp()` (`£` + `toLocaleString('en-GB')`). Also top-level `addOns` (sniffari/pick-up/drop-off — the day-school estimator's optional extras) and `flexiPass` (Five Days Flexi Pass £99.95 — the estimator's smart inline note; NOT an offering, would render a plan card; T&Cs live on Acuity by owner decision). Fix prices/facts in data, never in page copy.
