...
| 4b | `/comprehensive-puppy-training` (new, **off-nav**) | ☑ 2026-07-06 → **`docs/page-specs/04b-comprehensive-puppy-training.md`** | ◐ content+design+motion BUILT 2026-07-06 (trainer-led £1,200 puppy month; time-on/off signature; overnight-boarding/sep-anxiety + council badge; homepage+classes cross-sell repointed to it); owner preview + polish pending | ☐ |
...
4b. **`/comprehensive-puppy-training` (new, off-nav)** — Dedicated page for the £1,200 Comprehensive Puppy Training product (`puppy-intensive`), added 2026-07-06 at owner request. **Kept OUT of the nav** (nav-weight). **HOMEPAGE BUILD (last) must surface this from the homepage service options, not the nav** — the `puppy-intensive` card's `detailHref` already points here. Trainer-led/labour-intensive positioning vs the parent-led £175 classes; owns the "start-now window", named "time on/off" socialisation rhythm, and boarding-to-prevent-separation-anxiety angles. Spec: `docs/page-specs/04b-comprehensive-puppy-training.md`.
