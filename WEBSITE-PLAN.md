...
| 4b | `/comprehensive-puppy-training` (new, nav **"Puppy School"**) | ☑ 2026-07-06 → **`docs/page-specs/04b-comprehensive-puppy-training.md`** | ◐ content+design+motion BUILT 2026-07-06 (trainer-led £1,200 puppy month; time-on/off signature; overnight-boarding/sep-anxiety + council badge; homepage+classes cross-sell repointed; **in nav as "Puppy School"**, replacing the "Puppy Classes" nav item); owner preview + polish pending | ☐ |
| 5 | `/training-plans` (new) | ☑ 2026-07-06 → **`docs/page-specs/05-training-plans.md`** | ◐ BUILT + DEPLOYED + LIVE 2026-07-08 (commit `d523603`; filter chips + 9-plan grid + variety video hero); owner review (hero cut + DRAFT copy + real-phone check) pending | ☐ |
| 6 | `/membership-plans` (new) | ☑ 2026-07-11 → **`docs/page-specs/06-membership-plans.md`** | ◐ BUILT + DEPLOYED + LIVE 2026-07-11 (commit `be8c42d`, run `29145961588`; 3 tiers + derived savings counters, hero = animation-slot placeholder for the owner's coming design, grooming-regulars band, apply form; 5-agent adversarial review → 13 findings all fixed); owner live review + hero animation pending | ☐ |
| 7 | `/puppycourse` (immersive course hub) | ☑ 2026-07-12 → **`docs/page-specs/07-puppycourse.md`** | ◐ BUILT 2026-07-12 (3 signatures: week-chapter trail, sun-arc daily schedule, toilet-age tool; absorbs the 5 popups + resources-collection; CER keeps the `ConditionedEmotionalResponse` anchor; 6 stubs deep-link in; footer + blog-CTA + both pathway cards flipped); owner review pending | ☐ |
...
| 12 | `/` **Homepage — LAST** | ☑ 2026-07-02 → **`docs/page-specs/12-homepage.md`** | ◐ content+design+motion+WebGL built + LIVE (2026-07-02, `57d9a99`); **2026-07-04: hero upgraded to a 3-step sequence** (photo+storm → particles form the brand logo → `home-hero.mp4` fades in) + reassurance video loop + retagline + trust-strip council rating; soul content + polish + sign-off pending | ☐ |
| 13 | `/london` (**new — off-plan**; the first **D-track catchment page**, created by the SEO programme, not the original manifest) | ☑ 2026-07-14 → **`docs/page-specs/13-london.md`** | ◐ BUILT + DEPLOYED + LIVE 2026-07-14 (targets "residential dog training london" 100/mo KD15, where we sat #9 with no page; passes the SEO.md §1 doorway test on 5 sections; interlinked pillar ↔ /london ↔ the London-parks post); owner review pending (named London case studies + DRAFT copy + real-phone check) | ☐ |

**Catchment pages are gated:** `/london` is #1 of a **3–6 page cap** (SEO.md §1 locked decision #3, owner-signed 2026-07-13). Shortlist: `/london` → `/kent`; reserves `/essex`, `/surrey`. **Every catchment page must pass the doorway test (≥3 sections that could not be find-replaced to another town) and get its own pre-build interview** — find-replace county pages are doorway spam and are PROHIBITED.
...
4b. **`/comprehensive-puppy-training` (new, off-nav)** — Dedicated page for the £1,200 Comprehensive Puppy Training product (`puppy-intensive`), added 2026-07-06 at owner request. **In the main nav as "Puppy School"** (owner update 2026-07-06 — the primary puppy offer, replacing the "Puppy Classes" nav item; Puppy Training Classes stays reachable via internal links: homepage grid + the puppy pages' cross-sell). The `puppy-intensive` card's `detailHref` also points here. Trainer-led/labour-intensive positioning vs the parent-led £175 classes; owns the "start-now window", named "time on/off" socialisation rhythm, and boarding-to-prevent-separation-anxiety angles. Spec: `docs/page-specs/04b-comprehensive-puppy-training.md`.
...
6. **`/membership-plans` (new)** — 4 tiers (Half £275/mo, Full T1 £375/mo, Full T2 £750 bi-monthly, Grooming sub £25/mo); savings-vs-pay-per-day animated counters; CTA: "Apply for membership" enquiry variant. **⚠ Plan details owner-corrected 2026-07-04 — `pricing.json` is the single source of truth** (verbatim old-site cards + owner edits: T1 has NO free training session; T2 = TWO free sessions/cycle; free pick-ups & drop-offs ARE a T1/T2/grooming-sub perk — do NOT "reconcile" back to £1/journey, see `docs/page-specs/12-homepage.md`). On ship: flip the TEMP CTAs/redirects per the HANDOVER 🔔 flip-back checklist.
...
| **4 — Site-wide SEO + tracking finish** | JSON-LD everywhere, OG images, sitemap/robots, GA4 property + GTM tags (guided), Rich Results + Tag Assistant (both consent states), full `--dist` manifest pass *(execution tracked in `SEO.md` Stages 3–4)* | All checks green; final content read-through |
